...
if torch.cuda.is_available():
    st.sidebar.markdown("### CUDA 状态: 可用 ✅")
    # 如果需要，您可以进一步选择具体的 GPU
    torch.cuda.set_device(0)
else:
    st.sidebar.markdown("### CUDA 状态: 不可用 ❌")


MODEL_PATH = os.environ.get('MODEL_PATH', r'D:\1-PythonProjects\Skin_demo\ChatGLM3\chatglm3-6b')
...
def get_model():
    ...
    model = AutoModel.from_pretrained(MODEL_PATH, trust_remote_code=True).quantize(4).cuda()  ##todo
    model = model.eval()
